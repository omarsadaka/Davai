...
public class GetCity {
    ...
    public void getCity(String id, final Spinner userCity, final int layout) {
        cityList.clear();
        ...
        call.enqueue(new Callback<List<CityModel>>() {
            ...
            public void onResponse(Call<List<CityModel>> call, Response<List<CityModel>> response) {
                if (response.body() != null) {
                    ...
                    list.add(context.getResources().getString(R.string.select_city));
    ...
    public void getCity2(String id, final Spinner userCity, final int layout) {
        cityList.clear();
        ClientInterface clientInterface = ApiClient.getClient().create(ClientInterface.class);
        Call<List<CityModel>> call = clientInterface.getCities(id);
        call.enqueue(new Callback<List<CityModel>>() {
    ...
    public static int getId(String id){
        for (int i=0;i<cityList.size();i++){
            if (id== cityList.get(i).getId() || cityList.get(i).equals(id)){
                return i;
            }
        }
